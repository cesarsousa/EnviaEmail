...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:output method="xml" version="1.0" encoding="UTF-8" indent="yes" doctype-system="brf103-request-v2.5.dtd"/>
	...
	<xsl:template match="PG_PEDIDO">
		...
		<xsl:element name="info-processo">
			<xsl:element name="tipo-processo">
				<xsl:choose>
					<xsl:when test="TPN_PEDIDO/TPN_NATUREZA/RB_PATENTE_INVENCAO = $true">
						...
						<!--
						<xsl:choose>
							<xsl:when test="TPN_PEDIDO_DIVIDIDO/CB_PEDIDO_DIVIDIDO = $true">
								<xsl:value-of select="$patente-de-invencao-dividido"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$patente-de-invencao"/>
							</xsl:otherwise>						
					...
					<xsl:when test="TPN_PEDIDO/TPN_NATUREZA/RB_MODELO_UTILIDADE = $true">
						...
						<xsl:choose>
							<xsl:when test="TPN_PEDIDO_DIVIDIDO/CB_PEDIDO_DIVIDIDO = $true">
								<xsl:value-of select="$modelo-de-utilidade-dividido"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$modelo-de-utilidade"/>
							</xsl:otherwise>						
			...
			<xsl:element name="info-pct">
				<xsl:element name="pais-pct">
					<xsl:value-of select="TPN_PEDIDO/PNL_PCT/CCBB_PCT_NUM_PAIS"/>
				</xsl:element>
				<xsl:element name="pais-pct-extenso">
					<xsl:value-of select="TPN_PEDIDO/PNL_PCT/CCBB_PCT_NUM_PAIS/@content"/>
				</xsl:element>
				<xsl:element name="ano">
					<xsl:value-of select="TPN_PEDIDO/PNL_PCT/ED_PCT_NUM_ANO"/>
				</xsl:element>
				<xsl:element name="serial">
					<xsl:value-of select="TPN_PEDIDO/PNL_PCT/ED_PCT_NUM_SEQ"/>
				</xsl:element>
				<xsl:element name="data-pct">
					<xsl:if test="TPN_PEDIDO/PNL_PCT/EDD_PCT_DATA='18991230'">
						<xsl:value-of select="$vazio"/>
					</xsl:if>
					<xsl:if test="not(TPN_PEDIDO/PNL_PCT/EDD_PCT_DATA='18991230')">
						<xsl:value-of select="TPN_PEDIDO/PNL_PCT/EDD_PCT_DATA"/>
					</xsl:if>				
				</xsl:element>			
			</xsl:element>
			
			<!--
			<xsl:variable name="dividido">
				<xsl:value-of select="string(boolean(TPN_PEDIDO_DIVIDIDO/CB_PEDIDO_DIVIDIDO = $true))"/>
			</xsl:variable>
			-->
			
			<!--
			<xsl:element name="processo-relacionado">
				<xsl:attribute name="divisional">
					<xsl:value-of select="$dividido"/>
				</xsl:attribute>
				
				<xsl:element name="tipo">
					<xsl:choose>
						<xsl:when test="$dividido = $true">
							<xsl:value-of select="TPN_PEDIDO_DIVIDIDO/TPN_NUM_PEDIDO_DIVIDIDO/CB_SIG_NUM_DIVIDIDO"/>
						</xsl:when>
						<xsl:otherwise>
				...
				<xsl:element name="tipo-extenso">
					<xsl:choose>
						<xsl:when test="$dividido = $true">
							<xsl:value-of select="TPN_PEDIDO_DIVIDIDO/TPN_NUM_PEDIDO_DIVIDIDO/CB_SIG_NUM_DIVIDIDO/@content"/>
						</xsl:when>
						<xsl:otherwise>
				...
				<xsl:element name="numero">
					<xsl:choose>
						<xsl:when test="$dividido = $true">
							<xsl:value-of select="TPN_PEDIDO_DIVIDIDO/TPN_NUM_PEDIDO_DIVIDIDO/ED_NUM_PEDIDO_DIVIDIDO"/>
						</xsl:when>
						<xsl:otherwise>
			...
			<xsl:element name="periodo-de-graca">
				<xsl:call-template name="criar-documento">
					<xsl:with-param name="origem" select="TPN_PEDIDO/TPN_GRACA/PNL_GRACA"/>
				</xsl:call-template>
			</xsl:element>
	...
	<xsl:template name="criar-grus">
		<xsl:variable name="nome-requerente">
			...
			<xsl:if test="document/@documentcode = 'GRU'">
				<xsl:element name="gru">
					<xsl:attribute name="principal">
						<xsl:value-of select="string(boolean(PNL_ITEM_GRU/ED_GRU_PRINCIPAL = $true))"/>
					</xsl:attribute>
					...
					<xsl:attribute name="requerente">
						<xsl:choose>
							<xsl:when test="$nome-requerente = $vazio">
								<xsl:value-of select="PNL_ITEM_GRU/ED_GRU_NOME_DEP"/>						
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$nome-requerente"/>
							</xsl:otherwise>
						</xsl:choose>					
					</xsl:attribute>
				</xsl:element>
	...
	<xsl:template match="PG_PRIORIDADE">
		<xsl:element name="prioridades">
			<xsl:call-template name="criar-declaracao">
				...
				<xsl:with-param name="texto">Declaro que os dados identificadores do(s) pedido priorit&#225;rio cuja prioridade(s) &#233; aqui reivindicada est&#227;o em conformidade com ou contidos no requerimento de dep&#243;sito do presente pedido.</xsl:with-param>
			...
			<xsl:element name="documento">
				...
				<xsl:if test="$origem/document/@documentcode = 'GRU'">
					...
					<xsl:attribute name="gru-principal">
						<xsl:value-of select="string(boolean($origem/PNL_ITEM_GRU/ED_GRU_PRINCIPAL = $true))"/>
					</xsl:attribute>
				</xsl:if>			

				<xsl:if test="$origem/document/@documentcode = 'DESENHOS'">
	...
	<xsl:template name="obter-doc-code-final">
		...
		<xsl:choose>
			<xsl:when test="$doc-code = 'RELATDESC'"><xsl:value-of select="$ESPEC-BR"/></xsl:when>
			<xsl:when test="$doc-code = 'REIVIND'"><xsl:value-of select="$ESPEC-BR"/></xsl:when>
			<xsl:when test="$doc-code = 'RESUMO'"><xsl:value-of select="$ESPEC-BR"/></xsl:when>
			<xsl:when test="$doc-code = 'DESENHOS'"><xsl:value-of select="$ESPEC-BR"/></xsl:when>
