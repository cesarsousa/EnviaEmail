...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="requerente" select="'requerente'"/>
	...
	<xsl:variable name="patente-de-invencao" select="'patente de invencao pct'"/>
	<xsl:variable name="modelo-de-utilidade" select="'modelo de utilidade pct'"/>
	...
	<xsl:template match="info-processo">
		<xsl:element name="TPN_PEDIDO">
			<xsl:element name="TPN_NATUREZA">			
				<xsl:element name="RB_PATENTE_INVENCAO">
					<xsl:variable name="ehPatenteInvencao" select="boolean(tipo-processo = $patente-de-invencao) or boolean(tipo-processo = $patente-de-invencao-dividido)"/>
					<xsl:value-of select="string($ehPatenteInvencao)"/>
				</xsl:element>
				<xsl:element name="RB_MODELO_UTILIDADE">
					<xsl:variable name="ehModeloUtilidade" select="boolean(tipo-processo = $modelo-de-utilidade) or boolean(tipo-processo = $modelo-de-utilidade-dividido)"/>
				...
				<xsl:element name="CB_NUM_CERT_ADICAO">
					<xsl:if test="tipo-processo = $certificado-de-adicao">
						<xsl:value-of select="processo-relacionado[@divisional != $true]/tipo"/>
					</xsl:if>
				</xsl:element>
				<xsl:element name="ED_NUM_CERT_ADICAO">
					<xsl:choose>
						<xsl:when test="tipo-processo = $certificado-de-adicao">
							<xsl:value-of select="processo-relacionado[@divisional != $true]/numero"/>
						</xsl:when>
						<xsl:otherwise>
			...
			<!--
			<xsl:element name="LBL_PRIVILEGIO_LEI">
			</xsl:element> 
			-->
			...
			<xsl:element name="PNL_PCT">
				<xsl:element name="CCBB_PCT_NUM_PAIS">
					<xsl:attribute name="content">
						<xsl:value-of select="info-pct/pais-pct-extenso"/>
					</xsl:attribute>
					<xsl:value-of select="info-pct/pais-pct"/>				
				</xsl:element>
				<xsl:element name="ED_PCT_NUM_ANO">
					<xsl:value-of select="info-pct/ano"/>				
				</xsl:element>
				<xsl:element name="ED_PCT_NUM_SEQ">
					<xsl:value-of select="info-pct/serial"/>				
				</xsl:element>
				<xsl:element name="EDD_PCT_DATA">
					<xsl:value-of select="info-pct/data-pct"/>				
				</xsl:element>				
			</xsl:element>
			<xsl:element name="TPN_GRACA">
				<xsl:element name="PNL_GRACA">
					<xsl:attribute name="filecheckcontrol">CB_GRACA</xsl:attribute>
					<xsl:attribute name="documentcode">PERIODOGRACA</xsl:attribute>
					<xsl:attribute name="filenamecontrol">LBL_GRACA_DOCUMENTO</xsl:attribute>
					<xsl:attribute name="attachcontrol">BTN_GRACA</xsl:attribute>
					<xsl:attribute name="fileextension">Portable Document Format|*.pdf</xsl:attribute>
					<xsl:attribute name="renamefilename">PERIODOGRACA</xsl:attribute>
					<xsl:attribute name="multiple">true</xsl:attribute>
					
					<xsl:if test="periodo-de-graca/documento">
						<xsl:call-template name="criar-documents">
							<xsl:with-param name="origem" select="periodo-de-graca/documento"/>
						</xsl:call-template>
					</xsl:if>
					
					<xsl:element name="CB_GRACA">
						<xsl:value-of select="string(boolean(periodo-de-graca/documento))"/>
					</xsl:element>
				</xsl:element>
			</xsl:element>
		</xsl:element>
		...
		<!--
		<xsl:element name="TPN_PEDIDO_DIVIDIDO">
			<xsl:element name="CB_PEDIDO_DIVIDIDO">
				<xsl:value-of select="string(boolean(processo-relacionado/@divisional = $true))"/>
			</xsl:element>
			<xsl:element name="TPN_NUM_PEDIDO_DIVIDIDO">
				<xsl:element name="CB_SIG_NUM_DIVIDIDO">
					<xsl:value-of select="processo-relacionado[@divisional = $true]/tipo"/>
				</xsl:element>
				<xsl:element name="ED_NUM_PEDIDO_DIVIDIDO">
					<xsl:value-of select="processo-relacionado[@divisional = $true]/numero"/>
				</xsl:element>
			</xsl:element>
		</xsl:element>
	...
	<xsl:template match="participantes">
		<xsl:element name="CLCN_NOME">
			<xsl:element name="CLCN_DEPOSITANTE">
				<xsl:apply-templates select="requerentes/requerente"/>
			</xsl:element>
	...
	<xsl:template name="criar-pessoa-fisica-juridica">
		...
		<xsl:variable name="entidade">
			<xsl:choose>
				<xsl:when test="local-name() = $requerente">
		...
		<xsl:element name="CLCN_ITEM_{$titulo}">
			<xsl:if test="(local-name() = $requerente) and ($tipo-legal = $PF)">
			...
			<xsl:element name="PNL_{$titulo}_NOME">
				...
				<xsl:if test="(local-name() = $requerente)">
			...
			<xsl:if test="(local-name() = $requerente) and ($tipo-legal = $PJ)">
	...
	<xsl:template name="documentos-especificacao">
		<xsl:element name="PNL_ESPECIFICACAO_entity">
			...
			<xsl:variable name="ehResumo" select="boolean(secao[@doc-code = 'RESUMO'])" />
				...
				<xsl:element name="PNL_RESUMO_PAGINAS">
					<xsl:element name="ED_RESUMO_DE">
						<xsl:if test="$ehResumo">
							<xsl:value-of select="secao[@doc-code = 'RESUMO']/@pagina-inicial" />
						</xsl:if>
					</xsl:element>
					<xsl:element name="ED_RESUMO_PARA">
						<xsl:if test="$ehResumo">
							<xsl:value-of select="secao[@doc-code = 'RESUMO']/@pagina-final" />
						</xsl:if>
					</xsl:element>
				</xsl:element>
			</xsl:element>
			
			<xsl:variable name="ehRelatorioDescritivo" select="boolean(secao[@doc-code = 'RELATDESC'])" />
				...
				<xsl:element name="PNL_RELATORIO_DESCRITIVO_PAGINAS">
					<xsl:element name="ED_RELATORIO_DESCRITIVO_DE">
						<xsl:if test="$ehRelatorioDescritivo">
							<xsl:value-of select="secao[@doc-code = 'RELATDESC']/@pagina-inicial" />
						</xsl:if>
					</xsl:element>
					<xsl:element name="ED_RELATORIO_DESCRITIVO_PARA">
						<xsl:if test="$ehRelatorioDescritivo">
							<xsl:value-of select="secao[@doc-code = 'RELATDESC']/@pagina-final" />
						</xsl:if>
					</xsl:element>
				</xsl:element>
			</xsl:element>
			
			<xsl:variable name="ehReivindicacao" select="boolean(secao[@doc-code = 'REIVIND'])" />
				...
				<xsl:element name="PNL_REIVINDICACAO_PAGINAS">
					<xsl:element name="ED_REIVINDICACAO_DE">
						<xsl:if test="$ehReivindicacao">
							<xsl:value-of select="secao[@doc-code = 'REIVIND']/@pagina-inicial" />
						</xsl:if>
					</xsl:element>
					<xsl:element name="ED_REIVINDICACAO_PARA">
						<xsl:if test="$ehReivindicacao">
							<xsl:value-of select="secao[@doc-code = 'REIVIND']/@pagina-final" />
						</xsl:if>
					</xsl:element>
				</xsl:element>
			</xsl:element>
			
			<xsl:variable name="ehDesenho" select="boolean(secao[@doc-code = 'DESENHOS'])" />
				...
				<xsl:element name="PNL_DESENHO_PAGINAS">
					<xsl:element name="ED_DESENHO_DE">
						<xsl:if test="$ehDesenho">
							<xsl:value-of select="secao[@doc-code = 'DESENHOS']/@pagina-inicial" />
						</xsl:if>
					</xsl:element>
					<xsl:element name="ED_DESENHO_PARA">
						<xsl:if test="$ehDesenho">
							<xsl:value-of select="secao[@doc-code = 'DESENHOS']/@pagina-final" />
						</xsl:if>
					</xsl:element>
				</xsl:element>
			...
			<xsl:element name="PNL_ITEM_{$entidade}">
				...
				<xsl:choose>
					...
					<xsl:when test="@doc-code = 'GRU'">
						...
						<xsl:element name="ED_GRU_PRINCIPAL">
							<xsl:value-of select="string(boolean(@gru-principal = $true))"/>
						</xsl:element>
					</xsl:when>
					<xsl:when test="@doc-code-sec = 'INDEX-BR'">
