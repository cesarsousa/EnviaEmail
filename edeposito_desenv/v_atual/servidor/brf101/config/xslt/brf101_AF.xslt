...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="language" select="/BRF101/@language_of_proceedings"/>
	<xsl:template match="BRF101">
		<internal>
			<xml-documents>
				<doc infile="data.xml" xslfile="brf101-request.xslt" outfile="brf101-request.xml" signatory="false"/>
			...
			<pdf-documents>
				<doc infile="brf101-request.xml" outfile="brf101-request.pdf" signatory="true" id="0">
					<xsl:variable name="switch_lang_request_xfo">
						<xsl:call-template name="SwitchXFOfileName">
							<xsl:with-param name="FileName" select="'brf101-request.xfo'"/>
						</xsl:call-template>
					</xsl:variable>
	...
	<xsl:template match="CLCN_ESPECIFICACAO">
		<documents-info>
			...
			<xsl:apply-templates select="./PNL_ESPECIFICACAO_entity/document[@documentcode='RELATDESC']"/>
			<xsl:apply-templates select="./PNL_ESPECIFICACAO_entity/document[@documentcode='REIVIND']"/>
			<xsl:apply-templates select="./PNL_ESPECIFICACAO_entity/document[@documentcode='RESUMO']"/>
			<xsl:apply-templates select="./PNL_ESPECIFICACAO_entity/document[@documentcode='DESENHOS']"/>
		</documents-info>
	</xsl:template>
